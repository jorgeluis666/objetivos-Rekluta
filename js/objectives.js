(function () {
  ...
  // proyecto de origen. Rekluta arranca sin historico.
  const CLOSED_MONTH_URLS = [];
  // PENDIENTE: ID del Google Sheet de Rekluta. Vacio = sin sincronizacion en vivo.
  const SHEET_ID = '';
  ...
  const GOALS_KEY = 'rk-reservation-goals-v1';
  const SHEET_SYNC_ENDPOINT_KEY = 'rk-sheet-sync-endpoint-v1';
  ...
  const CHART_SERIES_KEY = 'rk-chart-series-v1';
  ...
  function syncEndpoint() {
    ...
    return (window.RK_SHEET_SYNC_ENDPOINT || localStorage.getItem(SHEET_SYNC_ENDPOINT_KEY) || '').trim();
  }

  // ── Custom campaigns / ads (stored in localStorage) ──────────────────────
  const CUSTOM_KEY = 'rk-custom-rows-v1';
  ...
  function sheetToMonthData(rows) {
    ...
    return {
      ...
      source: `Google Sheets / Rekluta / ${SHEET_MONTH}`
    };
  }
  ...
  async function syncLiveSheet({ silent = false, manual = false } = {}) {
    if (!SHEET_ID) {
      updateSyncLabel('Falta configurar el Google Sheet de Rekluta');
      return false;
    }
    if (manual) {
    ...
    try {
      ...
      if (!silent) console.warn('[rk] No se pudo sincronizar Google Sheets:', error);
  ...
  async function pushGoalToSheet(input) {
    ...
    try {
      ...
      console.warn('[rk] No se pudo enviar el objetivo a Google Sheets:', error);
  ...
  async function init() {
    ...
    } catch (error) { document.getElementById('view-obj').innerHTML = '<div class="data-notice error"><strong>No se pudo cargar la informacion de Rekluta.</strong></div>'; console.error(error); }
  }
