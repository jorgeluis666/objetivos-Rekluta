(function () {
  ...
  function renderHeader(projection) {
    ...
    if (note) {
      note.textContent = `La proyeccion asume que se mantiene el ritmo promedio del mes (${money(projection.byKey.investment.pace)} por dia). Fuente: ${projection.source || 'Rekluta'}.`;
    }
  ...
  function render() {
    // El modulo puede abrirse antes de que Gasto publicitario termine de cargar; el evento rk:data-updated lo reintenta.
    const snapshot = window.RKObjectives?.snapshot?.();
    if (!snapshot) {
      renderWaiting();
      return;
    }
